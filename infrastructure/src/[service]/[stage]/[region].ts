...
import * as utils_aws from "../../utils/aws.js";
import * as utils_domain from "../../utils/domain.js";
import * as utils_nix from "../../utils/nix.js";
import * as utils_platform from "../../utils/platform.js";

export const stacks = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): aws_cdk_lib.Stack => {
...
export const resourceGroups = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): aws_resourcegroups.CfnGroup => {
		const tags = utils_aws.tags({
			domainPath,
...
export const githubActionsOIDCProviders = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): aws_iam.CfnOIDCProvider | undefined => {
		const [, stage] = domainPath;
		if (stage.data.pipeline) {
			...
			return new aws_iam.CfnOIDCProvider(stacks(domainPath), name, {
				clientIdList: [aws_iam.ServicePrincipal.servicePrincipalName("sts")],
				tags: Object.entries(tags).map(([key, value]) => {
					return {
						key,
						value
...
export const githubActionsRoles = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): aws_iam.CfnRole | undefined => {
		const [, stage] = domainPath;
		if (stage.data.pipeline) {
			const tags = utils_aws.tags({
				domainPath,
...
export const testKeyPairs = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): aws_ec2.CfnKeyPair => {
		const tags = utils_aws.tags({
			domainPath,
...
export const imageBuilderLogGroups = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): aws_logs.CfnLogGroup => {
		const tags = utils_aws.tags({
			domainPath,
...
export const imageBuilderLogBuckets = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): aws_s3.CfnBucket => {
		const tags = utils_aws.tags({
			domainPath,
...
export const imageBuilderInstanceRoles = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): aws_iam.CfnRole => {
		const tags = utils_aws.tags({
			domainPath,
					...
					policyDocument: {
						...
						Statement: [
							...
							},
							{
								Effect: "Allow",
								Action: "s3:GetObject",
								Resource: stacks(domainPath).formatArn({
									service: "s3",
									region: "",
									account: "",
									resource: "*",
							...
							{
								Effect: "Allow",
								Action: ["imagebuilder:GetComponent", "imagebuilder:GetMarketplaceResource"],
								Resource: "*",
								Condition: {
							...
							// EC2 Instance Connect.
							{
								...
								Condition: {
									StringEquals: Object.fromEntries(
										...
										).map(([key, value]) => [`aws:ResourceTag/${key}`, value])
									)
								}
							},
							// Key pair.
...
export const imageBuilderInstanceProfiles = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): aws_iam.CfnInstanceProfile => {
		const tags = utils_aws.tags({
			domainPath,
		...
		return new aws_iam.CfnInstanceProfile(stacks(domainPath), name, {
			...
			roles: [imageBuilderInstanceRoles(domainPath).ref]
		});
	}
);

export const imageBuilderSecurityGroups = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): aws_ec2.CfnSecurityGroup => {
		const tags = utils_aws.tags({
			domainPath,
...
export const imageBuilderAllIpv4SecurityGroupEgresses = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): aws_ec2.CfnSecurityGroupEgress => {
		const tags = utils_aws.tags({
			domainPath,
...
export const imageBuilderAllIpv6SecurityGroupEgresses = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): aws_ec2.CfnSecurityGroupEgress => {
		const tags = utils_aws.tags({
 ...
 */
interface StrictLaunchTemplateDataProperty
	extends aws_ec2.CfnLaunchTemplate.LaunchTemplateDataProperty {
...
export const testManualLaunchTemplateData = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): ((platform: utils_platform.Platform) => StrictLaunchTemplateDataProperty) => {
		return utils_platform.mapPlatforms(
			utils_platform.platforms(domainPath),
			(platform): StrictLaunchTemplateDataProperty => {
				return {
					blockDeviceMappings: [
						{
							deviceName: (() => {
					...
					imageId: (() => {
						switch (platform.supervisor) {
							case utils_platform.Supervisor.AMAZON_LINUX:
								return service.app.node.getContext(
									stacks(domainPath).formatArn({
										service: "ssm",
										account: "",
										resource: "parameter",
					...
					securityGroupIds: [imageBuilderSecurityGroups(domainPath).attrGroupId],
					tagSpecifications: [
						{
 ...
 */
export const testManualLaunchTemplates = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): ((platform: utils_platform.Platform) => aws_ec2.CfnLaunchTemplate) => {
		return utils_platform.mapPlatforms(
			utils_platform.platforms(domainPath),
			(platform): aws_ec2.CfnLaunchTemplate => {
				const tags = utils_aws.tags({
					platform,
					domainPath,
...
export const testImageRecipes = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): ((platform: utils_platform.Platform) => aws_imagebuilder.CfnImageRecipe) => {
		return utils_platform.mapPlatforms(
			utils_platform.platforms(domainPath),
			(platform): aws_imagebuilder.CfnImageRecipe => {
				const tags = utils_aws.tags({
					platform,
					domainPath,
...
export const testInfrastructureConfigurations = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(
		domainPath
	): ((platform: utils_platform.Platform) => aws_imagebuilder.CfnInfrastructureConfiguration) => {
		return utils_platform.mapPlatforms(
			utils_platform.platforms(domainPath),
			(platform): aws_imagebuilder.CfnInfrastructureConfiguration => {
				const tags = utils_aws.tags({
					platform,
					domainPath,
				...
				return new aws_imagebuilder.CfnInfrastructureConfiguration(stacks(domainPath), name, {
					...
					instanceTypes: [launchTemplateData.instanceType],
...
export const testDocuments = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): ((platform: utils_platform.Platform) => aws_ssm.CfnDocument) => {
		return utils_platform.mapPlatforms(
			utils_platform.platforms(domainPath),
			(platform): aws_ssm.CfnDocument => {
				const tags = utils_aws.tags({
					platform,
					domainPath,
											...
											inputs: {
												...
												runCommand: [
													...
													[
														...
														"add",
														"nixpkgs",
														utils_nix.flakeReferenceToURL(
															utils_nix.getFlakeInput(service.nixFlakeMetadata, "nixpkgs")
														)
													].join(" "),
													[
														"nix",
														...
														"system-manager",
														utils_nix.flakeReferenceToURL(
															utils_nix.getFlakeInput(service.nixFlakeMetadata, "system-manager")
														)
													].join(" "),
													...
													// Run tests.
													`su -l ec2-user -c 'EC2_KEY_PAIR_PRIVATE_KEY_SSM_PARAMETER=${stacks(
														domainPath
													).formatArn({
														service: "ssm",
														resource: "parameter",
											...
											inputs: {
												...
												runCommand: [
													...
													[
														...
														"nix-darwin",
														utils_nix.flakeReferenceToURL(
															utils_nix.getFlakeInput(service.nixFlakeMetadata, "nix-darwin")
														)
													].join(" "),
													// Install Nix packages.
													[
											...
											inputs: {
												...
												runCommand: ((): string[] => {
													...
													return [
														...
														// Run tests.
														`$env:EC2_KEY_PAIR_PRIVATE_KEY_SSM_PARAMETER='${stacks(
															domainPath
														).formatArn({
															service: "ssm",
															resource: "parameter",
...
export const testWorkflows = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): ((platform: utils_platform.Platform) => aws_imagebuilder.CfnWorkflow) => {
		return utils_platform.mapPlatforms(
			utils_platform.platforms(domainPath),
			(platform): aws_imagebuilder.CfnWorkflow => {
				const tags = utils_aws.tags({
					platform,
					domainPath,
...
export const imageBuilderLifecycleRoles = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): aws_iam.CfnRole => {
		const tags = utils_aws.tags({
...
export const imageBuilderRoles = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): aws_iam.CfnRole => {
		const tags = utils_aws.tags({
					...
					policyDocument: {
						...
						Statement: [
							...
							},
							{
								...
								Condition: {
									StringEquals: {
										"aws:RequestTag/CreatedBy": ["EC2 Fast Launch", "EC2 Image Builder"]
									}
								}
							},
							{
							...
							},
							{
								...
								Condition: {
									StringEquals: {
										"aws:RequestTag/CreatedBy": ["EC2 Fast Launch", "EC2 Image Builder"],
										"ec2:CreateAction": ["CreateImage", "RunInstances"]
									}
								}
							},
							{
							...
							},
							{
								...
								Resource: [
									...
									...Array.from(utils_platform.platforms(domainPath)).map((platform): string =>
										stacks(domainPath).formatArn({
											service: "ssm",
											resource: "document",
							...
							{
								Effect: "Allow",
								Action: ["ssm:CreateAssociation", "ssm:DeleteAssociation"],
								Resource: [
									stacks(domainPath).formatArn({
										service: "ec2",
										region: "*",
...
export const testImages = utils_domain.mapDomainStratumPaths(
	utils_domain.SERVICE_DOMAIN,
	utils_domain.DomainType.REGION,
	(domainPath): ((platform: utils_platform.Platform) => aws_imagebuilder.CfnImage) => {
		return utils_platform.mapPlatforms(
			utils_platform.platforms(domainPath),
			(platform): aws_imagebuilder.CfnImage => {
				const tags = utils_aws.tags({
					platform,
					domainPath,
				...
				return new aws_imagebuilder.CfnImage(stacks(domainPath), name, {
					...
					infrastructureConfigurationArn:
						testInfrastructureConfigurations(domainPath)(platform).attrArn,
