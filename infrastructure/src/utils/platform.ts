...
function platformKey(platform: Platform): string {
	...
	return confbox.stringifyYAML(platform, {
 ...
 */
export function mapPlatforms<T>(
	platforms: Set<Platform>,
	f: (platform: Platform) => T,
): (platform: Platform) => T {
	const map: {
		readonly [key: string]: T;
	} = Object.fromEntries(
		Array.from(platforms).map((platform) => [platformKey(platform), f(platform)]),
	);
...
export const platforms = domain.mapDomainStratumPaths(
	domain.SERVICE_DOMAIN,
	domain.DomainType.REGION,
	(_) =>
		new Set([
			{
...
export const instanceTypes = domain.mapDomainStratumPaths(
	domain.SERVICE_DOMAIN,
	domain.DomainType.REGION,
	(domainPath) => {
