...
function checkTags(tags: Tags): Tags {
	if (
		!Object.entries(tags).every(
			([key, value]) =>
				key.match(/^[a-zA-Z0-9 .:_=+@\-/]{1,128}$/) &&
				value.match(/^[a-zA-Z0-9 .:_=+@\-/]{0,256}$/),
		)
	) {
...
export function tags({
	...
	if (platform !== undefined) {
		entries.push(
			...Object.entries(platform).map(
				([key, value]) =>
					[[namespace, "platform", key].join("."), String(value)] as [string, string],
			),
		);
	}
	if (domainPath !== undefined) {
		entries.push(
			...domainPath.map(
				(domain) =>
					[[namespace, "domain", domain.type].join("."), domain.data.name] as [string, string],
			),
		);
	}
	...
	if (time !== undefined) {
		entries.push([[namespace, "time"].join("."), time]);
	}
