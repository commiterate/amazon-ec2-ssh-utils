...
function checkDomain<T extends DomainType>(domain: Domain<T>): Domain<T> {
	...
	let domainStratum: Domain<DomainType>[] = [domain as Domain<DomainType>];
		...
		const hasUniqueSubdomainNamesPerDomain = domainStratum
			.map(
				(domain) =>
					...
					new Set(domain.subdomains.map((subdomain) => subdomain.data.name)).size,
			)
			.every((_) => _);
		if (
			...
			throw new TypeError(
				"Domain does not have unique domain strata types and or absolute domain names!",
			);
		}
	}
	return domain;
		...
		if (domainStratumPaths.length > 0) {
			if (domainStratumPaths.at(-1)?.at(-1)?.type === domainStratumType) {
				map = Object.fromEntries(
					domainStratumPaths.map((domainPath) => [
				...
				domainStratumPaths = domainStratumPaths.flatMap((domainPath) => {
					return (domainPath.at(-1) as unknown as Domain<DomainType>).subdomains.map(
						(subdomain) => [...domainPath, subdomain],
...
export function trimDomainPath<T extends DomainType>(
	...
	while (
		trimmedDomainPath.length > 1 &&
		(trimmedDomainPath.at(-1) as Domain<DomainType>).type !== domainStratumType
	) {
		trimmedDomainPath = trimmedDomainPath.slice(
			0,
			trimmedDomainPath.length - 1,
		) as unknown as DomainPath<DomainType>;
	}
