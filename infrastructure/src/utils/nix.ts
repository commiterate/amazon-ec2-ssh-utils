 ...
 * @returns Flake URL.
 */
export function flakeReferenceToURL<T extends FlakeType>(
	flakeReference: FlakeReference<T>,
): string {
	...
	const params = new Map(
		Object.entries(flakeReference)
			.filter(([key]) => !nonParamKeys.has(key))
	...
	switch (flakeReference.type) {
		case FlakeType.PATH:
			...
			["path"].forEach((key) => baseUrlParamKeys.add(key));
			break;
		case FlakeType.FILE:
		case FlakeType.TARBALL:
		case FlakeType.GIT:
			const gitFlakeReference = flakeReference as FlakeReference<
				FlakeType.FILE | FlakeType.TARBALL | FlakeType.GIT
			>;
			baseUrl = `${gitFlakeReference.type}+${gitFlakeReference.url}`;
			["url"].forEach((key) => baseUrlParamKeys.add(key));
			break;
		case FlakeType.GITHUB:
		case FlakeType.GITLAB:
			const gitlabFlakeReference = flakeReference as FlakeReference<
				FlakeType.GITHUB | FlakeType.GITLAB
			>;
			/*
			 * `%2F` is URL-encoded `/`.
			 *
			...
			["owner", "repo", "rev", "ref"].forEach((key) => baseUrlParamKeys.add(key));
			break;
	}
	const query = new Map(params.entries().filter(([key]) => !baseUrlParamKeys.has(key)));
	if (query.size > 0) {
		baseUrl +=
			"?" +
			query
...
export function getFlakeMetadata(url?: string): FlakeMetadata {
	...
	return JSON.parse(child_process.execSync(command.join(" ")).toString("utf-8"));
}
 ...
 */
export function getFlakeInput(
	flakeMetadata: FlakeMetadata,
	input: string,
): FlakeReference<FlakeType> {
	if (input === "self") {
